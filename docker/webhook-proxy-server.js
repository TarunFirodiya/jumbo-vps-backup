...
async function processPortalEnquiry(source, payload) {
  ...
  // Resolve the active JUM682 allocation before creating the enquiry.
  // Keep the legacy default only when the building has no active zone agent.
  const zoneAgent = building?.id ? await findZoneAgentForBuilding(building.id) : null;
  const initialAgentId = zoneAgent?.agentId || AASHISH_WORKSPACE_MEMBER_ID;
  const enquiry = await createEnquiry(
    payload, buyer.id, listing?.id, building?.id, property?.id,
    buyer.name, payload.phoneDigits, initialAgentId
  );
  console.log(`  Created enquiry: ${enquiry.id} (assigned to ${zoneAgent?.agentName || 'Aashish fallback'})`);
  await commitPortalEnquiry(dedupKey, enquiry.id);

  if (!zoneAgent && building?.id) {
    console.log(`  [Reassign] No active JUM682 agent for building ${building.name} — enquiry remains on fallback`);
  }
